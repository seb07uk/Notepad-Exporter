...
root = tk.Tk()
root.title("Notepad Exporter v1.0")
root.geometry("800x600")

text_area = tk.Text(root, wrap="word", undo=True)
text_area.pack(expand=True, fill="both")

status_bar = tk.Label(root, text="Characters: 0", anchor="e")
...
def export_to(extension):
    ...
    default_name = f"export_{now}.{extension}"
    ...
    if file_path:
        ...
        try:
            ...
            with open(file_path, "w", encoding="utf-8") as file:
                file.write(content)
            messagebox.showinfo("Success", f"Saved as {os.path.basename(file_path)}")
        except Exception as e:
...
def export_all_formats():
    ...
    try:
        for ext, data in formats.items():
            file_path = os.path.join(folder, f"export_{now}.{ext}")
            with open(file_path, "w", encoding="utf-8") as file:
                file.write(data)
        messagebox.showinfo("Success", f"All formats saved in:\n{folder}")
    except Exception as e:
        messagebox.showerror("Error", f"Cannot save files:\n{e}")

def clear_text():
    if messagebox.askyesno("Confirmation", "Are you sure you want to clear all text?"):
        text_area.delete(1.0, tk.END)
        update_char_count()
...
export_menu.add_command(label="Preview Export", command=preview_export)
export_menu.add_command(label="Export to All Formats", command=export_all_formats)
for ext in ["txt", "doc", "bat", "vbs", "ps1", "py", "reg", "html", "md", "rtf"]:
    export_menu.add_command(label=f"Export to {ext.upper()}", command=lambda e=ext: export_to(e))
...
theme_menu.add_command(label="Neutral", command=lambda: apply_theme("neutral"))
